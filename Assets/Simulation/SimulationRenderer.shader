Shader "Hidden/SimulationRenderer"
{
    ...
    SubShader {
        ...
        Pass {
            ...
            float ComputeCellValue(int gx, int gy)
            {
                if (gx < 0 || gy < 0 || gx >= _GridWidth || gy >= _GridHeight)
                    return 0.0; // out-of-bounds -> zero weight
            ...
            float4 SmoothCornerColor(int gridX, int gridY, int cx, int cy)
            {
                ...
                for (int dy = -r; dy <= r; dy++) {
                    for (int dx = -r; dx <= r; dx++) {
                        float c = ComputeCellValue(gridX + cx + dx, gridY + cy + dy);
                        weight += 1.0;
                        lerpValue += c;
                    }
                }
                if (weight > 0){
                    lerpValue /= weight;
                    float dither = GetBayerDither(float2(gridX, gridY)) * _DitherStrength;
                    lerpValue = saturate(lerpValue + dither);
                    lerpValue = floor(lerpValue * _PosterizeLevels) / _PosterizeLevels; // posterize
